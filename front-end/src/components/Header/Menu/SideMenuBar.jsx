    ...
    <div onClick={()=>{setIsOpen(!isOpen)}}>
                  ...
                  {isOpen && (
                    <div className="navbar">
                        <svg className='svgsideMenu' width="70px" height="70px" strokeWidth="1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" 
                        ...
                        <div className={`nav_items ${isOpen && "open"}`}>  
                          {data.token == '' ? (
                            <>
                              <Link to={'/signup'} className='linkSideMenu'>Crear cuenta</Link>
                              <hr className='hrSideMenu' style={{borderColor:'black'}}/>      
                              <Link to={'/login'} className='linkSideMenu'>Iniciar sesión</Link>
                            </>
                          ):(
                            <Link to={'/'}  onClick={()=>setData(data.valid=false)} className='linkSideMenu'>Cerrar sesión</Link>
                          )}
                          { data.roles.length == 1 ? <Link to={'/reservas'} className='linkSideMenu'>Reservas</Link>: ""}
                          {data.roles[1] ? (<Link to={'/administration'} className='linkSideMenu'>Administración</Link>) : "" }
                        </div>
